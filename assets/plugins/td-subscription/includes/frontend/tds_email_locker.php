...
class tds_email_locker {
	...
	public function __construct() {
		...
		add_shortcode( 'tds_content_locker', function ( $atts, $content, $shortcode_tag ) {
			...
			// read locker id
			//$tds_locker_id = !empty( $atts['tds_locker_id'] ) ? (int) $atts['tds_locker_id'] : false;

			// return here if the locker id is not set
			//if ( !$tds_locker_id )
				//return '';

			//$buffy = '<span style="color: orangered;"> -->> locked content ( aka content locker ) shortcode render <<-- </span>';
			//$buffy .= '<pre>' . print_r( $atts, true ) . '</pre>';
			//$buffy .= '<pre>' . print_r( $content, true ) . '</pre>';
	...
	public function locker_init() {
		...
		if ( is_archive() || !$post || !class_exists( 'td_util' ) ) {
			return;
		}
