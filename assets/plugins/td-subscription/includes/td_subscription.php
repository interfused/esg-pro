...
//        tds_print_tasks();
				...
				if ( isset( $current_screen->id ) && $current_screen->id === 'edit-tds_email' ) {
                     if ( 'top' === $which ) {
                        ...
                        if ( $query ) { ?>
                            ...
                            <?php $url .= '?' . $query . '&tds_action=download_csv_file';
	...
	static function create_db() {
	    try {
		    ...
		    maybe_add_column(
                'tds_trackings',
                'notes',
                'ALTER TABLE `tds_trackings` ADD COLUMN `notes` VARCHAR(255) NOT NULL DEFAULT "" AFTER `page_html`'
            );
            ...
            maybe_create_table( 'tds_subscriptions', 'CREATE TABLE `tds_subscriptions`(
                ...
                `stripe_payment_intent` VARCHAR(40) DEFAULT "",
                `stripe_payment_status` VARCHAR(40) DEFAULT "",
	...
	public static function setup_environment() {
        ...
        add_action( 'manage_post_posts_custom_column' , function ( $column, $post_id ) {
            switch ( $column ) {
                case 'locked' :
                    $td_post_settings = td_util::get_post_meta_array( $post_id, 'td_post_theme_settings' );
                    $tds_lock_content = !empty( $td_post_settings['tds_lock_content'] );
                    $tds_locker_id = !empty( $td_post_settings['tds_locker'] ) ? $td_post_settings['tds_locker'] : false;
        ...
        add_action( 'template_redirect', function() {
            ...
            if (!is_null($my_account_page_id) && is_page($my_account_page_id)) {
	            ...
	            if ( ! wp_verify_nonce( $_nonce, 'save_account_details' ) ) {
		            return;
	            }
        ...
        add_action( 'login_init', function() {
            ...
            if ( is_plugin_active('td-subscription/td-subscription.php') && isset( $_GET['action'] ) && 'tds_validate' === $_GET['action'] ) {
