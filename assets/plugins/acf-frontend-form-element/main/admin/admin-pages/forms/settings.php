...
class Frontend_Forms_UI {
	...
	function display_shortcode() {
		...
		if ( 'admin_form' == $type || 'admin_template' == $type ){
			...
			foreach( array( 'form', 'template' ) as $content ){	
				...
				// Show shortcode
				$form_shortcode = sprintf( '<code>[%s %s="%d"]</code>', 'frontend_admin', $content, $post_id );
	...
	function columns_content( $column, $post_id ) {
		...
		if ( 'shortcode' == $column ) {
			...
			// Show shortcode
			echo sprintf( '<code>[%s %s="%d"]</code>', 'frontend_admin', $content, $post_id );
	...
	*/
	...
	function switch_forms_names_with_excerpts(){
		if( get_option( 'fea_version_3.5' ) ) return;
